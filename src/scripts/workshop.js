...
import {scrollProgress,scrollPosition,ScrollSmoother} from './scroll-journey.js';
...
let roomPlayer,stairPlayer,roomScroll,stairScroll,copyIndex=-1,scrollDriven=false,lastScrollY=scrollY,ticking=false;
...
function preparePlayers(){
  roomPlayer=new BufferedJourney(roomVideos,[manifest.videos.entry,manifest.videos.workbench],{
    ...
    onState(state){
      ...
      if(state==='error')roomScroll?.stop();
...
$('#play-room').addEventListener('click',()=>{scrollDriven=false;roomScroll?.stop();roomPlayer?.toggle();});
roomSlider.addEventListener('input',()=>{scrollDriven=false;roomScroll?.stop();roomPlayer?.seek(Number(roomSlider.value)/100);});
root.querySelectorAll('[data-room-stop]').forEach(el=>el.addEventListener('click',()=>{scrollDriven=false;roomScroll?.stop();roomPlayer?.seek(Number(el.dataset.roomStop));}));
...
function jumpDoor(event){
  event?.preventDefault();stairsSkipped=true;stairScroll?.stop();stairPlayer?.stop();
...
function scrollUpdate(){
  ...
  if(roomEnabled){
    ...
    if(rect.top<=header+32&&rect.bottom>header+80)roomScroll.by(delta/Math.max(innerHeight,room.offsetHeight-stage.offsetHeight));
  }
  ...
  if(stairsEnabled&&!stairsFailed&&!stairsSkipped&&rect.top<innerHeight&&rect.bottom>header){
    ...
    if(!stairPlayer.slots.size||Math.abs(p-stairPlayer.progress)>.001)stairScroll.to(p);
  }
}
...
root.addEventListener('click',event=>{if(event.target.closest('a[href^="#"]')){scrollDriven=false;roomScroll?.stop();stairScroll?.stop();roomPlayer?.pause();stairPlayer?.pause();}});
document.addEventListener('visibilitychange',()=>{if(document.hidden){roomScroll?.stop();stairScroll?.stop();roomPlayer?.pause();stairPlayer?.pause();allVideos.forEach(v=>v.pause());}});
