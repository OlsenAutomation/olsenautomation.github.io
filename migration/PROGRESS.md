...
Reproduced the old 375×667 failure: Enter started playback, and the stage/section both measured 558px with position:relative. The new stage is sticky at 599px and section travel is 2668px. Scene position now follows absolute document position, preserving reverse travel and fast flicks. One decoder seek is outstanding at a time; completed frames remain visible while the latest target changes. A 35,716-byte first-frame poster removes the initial backward jump. Original videos, portal/reference source and frozen inventories are retained.

Release checks pass; 36 candidate pages pass at 1440×900 and 390×844 (72 renders). Additional 375×667, 320×568 and 844×390 checks cover the pinned scene and invitation fit. Rapid/reverse gestures under 4× CPU emulation retain a single visible scene and paused videos; keyboard scrolling, idle stability, signup focus, reduced motion, no JavaScript and failed-media fallbacks pass. This is browser/emulated-input evidence, not physical Safari acceptance. Notification testing stays deferred, with no real messages, main, DNS or production changes. Published from `a4eee2b` as silent candidate `fad13e0e-302a-4b9e-8aa0-7eefa1f69ca4`. All 492 hosted files, 25 exclusions and 28 media ranges pass. Hosted phone checks verify direct entry, paused room movement, the final door CTA and signup focus.

Read-only remote check also found main at `7bbb4c7`, with newer intake-route and shared receiver changes after the frozen baseline. They were not copied into this preview or overwritten; reconciliation is recorded as required before launch. This turn made no main merge or modification.
...
Desktop, mobile and short landscape checks pass, including reverse movement, keyboard seeking, CTA focus, static/reduced-motion access, failed-media retry and no horizontal overflow. Full preservation/build/link/privacy/media checks pass. The other 34 public HTML pages match the previous hosted receipt; original assets and frozen inventories remain intact. See QA_CONTINUOUS_BASEMENT.md. Source commit `70e8868` is pushed and published at the existing branch alias as Worker version `5710c393-e50c-4e2a-a239-a7e655723b79`. All 425 hosted file hashes, three aliases, 20 exclusions and 24 video-range checks pass. Hosted mobile scrolling reversed to 63% then reached the door at 100%; the CTA focused the interest form. Main, GitHub Pages, DNS, custom domain and production are unchanged.
...
The 35 public pages and four isolated pages pass desktop/mobile checks (78 settled renders). Video playback, intent, keyboard focus, reduced motion, retry, no-script access and media-range checks pass. Full source history and attachments remain local-only. No original inventory item was deleted or moved. See PROJECT_STORY_CHECKPOINT.md and QA_PROJECT_STORIES.md. Branch-preview publication is complete: source commit `ef7e28f`, Worker version `c55c0803-7490-434c-a2df-cf71d505c8a3`. All 424 hosted file hashes, three aliases, 20 exclusions and 24 video-range checks pass. The existing preview alias points to this version. Remote main remains at the recorded baseline; production/domain routing is unchanged.
