...
Web, mail and SPF TTLs observed: 3600 seconds; NS 21600 seconds. Apex AAAA, CAA, DS and `_dmarc` TXT queries returned no answers. Initial public queries were not a complete zone export. Brian then opened Squarespace Domains in Safari. Read-only inspection captured all nine configured records (eight custom records plus Domain Connect), including the exact DKIM key; authoritative DNS queries cross-checked that record set. DNSSEC is disabled in the registrar UI. Raw timestamped DNS/table snapshots remain ignored locally. Refresh this record set immediately before cutover.
...
2. Add/review a Cloudflare **Free** full DNS zone, preserving every existing mail/verification/subdomain record. Keep apex A records and www CNAME pointing to GitHub Pages during DNS onboarding. Do not rely on automatic DNS scanning as a captured record set.
