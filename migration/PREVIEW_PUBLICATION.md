...
GitHub branch push and read-back completed. The published implementation is `16639b74917769bb3c2577e4b8015faac02cf060`; later documentation/test-only commits record its QA without changing the published files. Main was read back at the unchanged baseline `0feac3f8376c5110eedfc20f5cdeecd0494f1ed7`. GitHub Pages still reports `source.branch: main`, root path, `cname: olsenautomation.com`, status `built`. No tag was pushed.
...
- Exact verified URL: https://27859d44-olsen-automation-v2-preview.brian-dbb.workers.dev/
- Version: `27859d44-20fa-4ed1-bbe8-d2ac5061a735`, created 2026-09-13T20:50:01.311335Z, tag `16639b749177`.
- Version API read-back confirms the exact source-commit annotation, alias and media-only Worker routing. The preview-only configuration remains unchanged; the earlier endpoint read-back reported `enabled: false`, `previews_enabled: true`.

Version upload requires an existing Worker. A one-time bootstrap `wrangler deploy --config wrangler.preview.json` created version `9b94a3a4-c741-4059-bcdb-6c21d259e9a7` with no deployment targets. Version `b7880bf4-ff51-41a1-8dc6-e64cf9983f94` then exposed a real media seek failure. Version `29e16c24-e5ed-4578-8e94-5d2455b172c0` fixed range handling. The current version above also includes Brian’s basement usability corrections and a repeat hosted QA pass. Those older versions are historical, not the accepted branch preview.
...
- Asset directory: `dist` only, 343 published/configuration files, approximately 23.5 MB. `dist-unlisted`, source, migration documents, original handoff and archives are excluded from that directory.
- Exact local upload file/size/SHA-256 list: `.migration-local/basement-fix/upload-manifest.json` (ignored local evidence).
...
- All 341 served assets (343 published/configuration files minus `_headers` and `_redirects`) match source SHA-256. Full hosted QA and limits are recorded in `QA_HOSTED_PREVIEW.md`.
