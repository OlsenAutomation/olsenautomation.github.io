...
const mime = {'.html':'text/html; charset=utf-8','.css':'text/css','.js':'text/javascript','.json':'application/json','.svg':'image/svg+xml','.webp':'image/webp','.jpg':'image/jpeg','.png':'image/png','.mp4':'video/mp4','.txt':'text/plain'};
const server = http.createServer(async (req,res) => {
  try {
    if (!['GET','HEAD'].includes(req.method)) {res.writeHead(405);res.end();return;}
    const url = new URL(req.url, 'http://localhost');
    let pathname = decodeURIComponent(url.pathname);
    if (pathname === '/') {res.writeHead(302, {Location:'/preview/shell/'});res.end();return;}
    let file;
    ...
    if (pathname === '/reference/v6') file = path.join(root,'_codex_handoff/approved-pilot/Olsen-Automation-Future-Pilot-v6-Single-File.html');
      ...
      try {if (!(await realpath(file)).startsWith(dist+path.sep)) throw new Error('Outside output');await stat(file);}
    ...
    if (!pathname.startsWith('/reference/')) res.setHeader('Content-Security-Policy',"default-src 'self'; img-src 'self'; media-src 'self'; script-src 'self'; style-src 'self'; connect-src 'self'; object-src 'none'; base-uri 'none'; frame-ancestors 'none'; form-action 'none'");
...
server.listen(port,'127.0.0.1',()=>console.log(`Local preview: http://127.0.0.1:${port}/preview/shell/`));
