...
assert.match(base.hostname,/^(?:[a-f0-9]{8}|site-redesign-v2|release-candidate)-olsen-automation-v2-preview\.brian-dbb\.workers\.dev$/);
...
const files=(await walk(path.join(root,'dist-candidate'))).filter(file=>!['_headers','_redirects'].includes(path.basename(file)));
async function verify(file){
  const route='/'+path.relative(path.join(root,'dist-candidate'),file).split(path.sep).join('/');
  try{
    ...
    if(route.endsWith('.html'))assert.match(response.headers.get('content-security-policy')||'',route==='/ai-visibility.html'?/form-action https:\/\/script.google.com/:/form-action 'none'/);
...
const alerts=await fetch(new URL('/api/visit',base),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({event:'visit',route:'/index.html'})});
report.notifications={status:alerts.status,...await alerts.json()};
if(alerts.status!==200||report.notifications.state!=='disabled')report.failures.push({route:'/api/visit',error:'Preview alerts must be disabled'});
const manifest=JSON.parse(await readFile(path.join(root,'dist-candidate/media/manifest.json'),'utf8'));
report.ranges=[];
for(const video of Object.values(manifest.videos).flatMap(v=>v.variants)){
  const source=await readFile(path.join(root,'dist-candidate',video.url));
...
await writeFile(path.join(root,'.migration-local/release-candidate/hosted-http-checks.json'),JSON.stringify(report,null,2)+'\n');
console.log(JSON.stringify({base:report.base,assets:report.assets.length,aliases:report.routes.length,excluded:report.excluded.length,post:report.postStatus,notifications:report.notifications,ranges:report.ranges.length,failures:report.failures},null,2));
