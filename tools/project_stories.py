...
def videos(slug, media):
    ...
    clips = [
        ('whale-v041-motion', 'See the whale move', '12-second motion excerpt', 'Brian turns the hand crank on the assembled v0.4.1 prototype. The whale and wave supports move together. This is a normal-speed excerpt of the supplied video, with incidental audio removed.'),
        ('whale-v041-assembly', 'Watch the assembly', '29-second assembly timelapse', 'The supplied timelapse follows the physical assembly on the workbench. Parts are fitted and adjusted before the complete whale is shown. The original timelapse speed is retained.')
    ]
    output = []
    for identity, title, duration, description in clips:
        ...
        output.append(f'<figure class="project-clip" data-project-clip><h3 id="{identity}-title">{title}</h3><video width="{large["width"]}" height="{large["height"]}" playsinline preload="none" poster="{esc(poster["url"])}" aria-labelledby="{identity}-title" aria-describedby="{identity}-description" tabindex="0" data-small="{esc(small["url"])}" data-large="{esc(large["url"])}"></video><div class="clip-actions"><button class="btn primary" type="button" data-clip-play hidden>Play {duration}</button><a href="{esc(large["url"])}">Open MP4 <span class="clip-size">({large["bytes"]/1000000:.1f} MB)</span></a></div><p class="clip-status" role="status" aria-live="polite"></p><figcaption id="{identity}-description">{description}</figcaption></figure>')
    return '<section class="project-videos" aria-label="September 14 build videos"><p class="eyebrow">v0.4.1 · September 14, 2026</p><h2>From printed parts to motion.</h2><p>Two original build recordings. Press Play to load a clip.</p><div class="project-video-grid">' + ''.join(output) + '</div></section>'
