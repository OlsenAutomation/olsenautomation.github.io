...
if(config.main!=='src/worker.js'||config.assets.binding!=='ASSETS'||JSON.stringify(config.assets.run_worker_first)!=='["/media/*.mp4","/api/visit"]')throw Error('Unexpected media/notification Worker configuration.');
if(config.vars?.VISIT_NOTIFICATIONS!=='disabled')throw Error('Preview notifications must stay disabled.');
